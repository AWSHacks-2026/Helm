...
DEFAULT_STATIC_OUTPUT_DIR = Path("overlord/demo/generated/static-commerce")
DEFAULT_RICH_STATIC_OUTPUT_DIR = Path("overlord/demo/generated/static-commerce-rich")
StaticSiteMode = Literal["without-overlord", "with-overlord"]
...
def generate_agent_site(
    ...
    *,
    ...
    logger.info(
        ...
        quality_mode,
        max_tokens,
        os.getenv("OVERLORD_MOCK_BEDROCK") == "1",
        ",".join(task.sections),
    )
...
def build_coordination_plan(
    *,
    ...
    if not raw_usage:
        raise RuntimeError("Overlord static coordination did not return Bedrock usage")
    usage = InvokeUsage(
    ...
    *,
    ...
    for payload in generated:
        ...
        if mode == "with-overlord":
            ...
            warnings.extend(f"overlord deduped section: {section}" for section in sorted(removed))
        else:
    ...
    merge_conflicts = 0
    if quality_mode == "rich" and mode == "without-overlord" and len(generated) > 1:
        ...
        files["README.md"] += (
            f"\nThis rich without-Overlord run records {merge_conflicts} simulated "
...
def run_static_site_mode(
    *,
    ...
    started = time.perf_counter()
    coordinated = mode == "with-overlord"
...
def run_static_site_benchmark(config: StaticSiteConfig) -> list[StaticBenchmarkResult]:
    ...
    for agent_count in config.agent_counts:
        for mode in ("without-overlord", "with-overlord"):
...
def save_static_benchmark_figure(results: list[StaticBenchmarkResult], output_dir: Path) -> Path:
    ...
    axes[0].plot(agent_counts, values("without-overlord", "quality_score"), marker="o", label="Without Overlord")
    axes[0].plot(agent_counts, values("with-overlord", "quality_score"), marker="o", label="With Overlord")
    ...
    axes[1].plot(agent_counts, values("without-overlord", "tokens"), marker="o", label="Without Overlord")
    axes[1].plot(agent_counts, values("with-overlord", "tokens"), marker="o", label="With Overlord")
