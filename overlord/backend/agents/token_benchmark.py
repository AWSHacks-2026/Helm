...
DEFAULT_OUTPUT_DIR = Path("overlord/demo")
...
class BenchmarkRow:
    agent_count: int
    without_overlord_tokens: int
    with_overlord_tokens: int
    without_overlord_seconds: float
    with_overlord_seconds: float
    without_overlord_build_rate: float
    with_overlord_build_rate: float
    without_overlord_conflicting_edits: int
    with_overlord_conflicting_edits: int
    without_overlord_reverted_commits: int
    with_overlord_reverted_commits: int
...
def build_overlord_agents(agent_count: int) -> tuple[dict, dict]:
...
def arbitrate(agent_a: dict, agent_b: dict, **kwargs) -> dict:
    from overlord import arbitrate as overlord_arbitrate
...
def _usage_from_arbitration_result(result: dict, *, allow_mock: bool = False) -> InvokeUsage:
    ...
    if not raw_usage:
        if allow_mock and os.getenv("OVERLORD_MOCK_BEDROCK") == "1":
            payload = json.dumps(result)
            reasoning = str(result.get("reasoning", "mock Overlord arbitration"))
            return InvokeUsage(
                model_id="mock-overlord",
                role="overlord",
    ...
    _require_usage_tokens(usage, source="Overlord arbitration")
    return usage
...
def run_benchmark_matrix(config: BenchmarkConfig) -> list[BenchmarkRow]:
    if os.getenv("OVERLORD_MOCK_BEDROCK") == "1" and not config.allow_mock:
    ...
    for agent_count in config.agent_counts:
        ...
        rows.append(
            BenchmarkRow(
                agent_count=agent_count,
                without_overlord_tokens=int(without_overlord["tokens"]),
                with_overlord_tokens=int(with_overlord["tokens"]),
                without_overlord_seconds=float(without_overlord["seconds"]),
                with_overlord_seconds=float(with_overlord["seconds"]),
                without_overlord_build_rate=float(without_overlord["build_rate"]),
                with_overlord_build_rate=float(with_overlord["build_rate"]),
                without_overlord_conflicting_edits=int(
                    without_overlord["conflicting_edits"]
                ),
                with_overlord_conflicting_edits=int(
                    with_overlord["conflicting_edits"]
                ),
                without_overlord_reverted_commits=int(
                    without_overlord["reverted_commits"]
                ),
                with_overlord_reverted_commits=int(
                    with_overlord["reverted_commits"]
                ),
            )
        )
    return rows
...
def save_benchmark_figure(
    ...
    *,
    ...
    with_tokens = [row.with_overlord_tokens for row in rows]
    without_seconds = [row.without_overlord_seconds for row in rows]
    with_seconds = [row.with_overlord_seconds for row in rows]

    fig, axes = plt.subplots(1, 2)
    axes[0].plot(agent_counts, without_tokens, label="Without Overlord")
    ...
    axes[1].plot(agent_counts, without_seconds, label="Without Overlord")
...
def print_summary_table(rows: list[BenchmarkRow]) -> None:
    ...
    for row in rows:
        values = [
            row.agent_count,
            row.without_overlord_tokens,
            row.with_overlord_tokens,
            f"{row.without_overlord_seconds:.2f}",
            f"{row.with_overlord_seconds:.2f}",
            f"{row.without_overlord_build_rate:.2f}",
            f"{row.with_overlord_build_rate:.2f}",
            row.without_overlord_conflicting_edits,
            row.with_overlord_conflicting_edits,
            row.without_overlord_reverted_commits,
            row.with_overlord_reverted_commits,
        ]
