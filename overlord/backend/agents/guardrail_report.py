...
def render_markdown(result: dict[str, Any]) -> str:
    o = result.get("overlord") or {}
    if "comparison" not in result:
        usage = o.get("usage") or {}
        return "\n".join(
            [
                "# Overlord guardrail fleet benchmark",
                "",
    ...
    return "\n".join(
        [
            "# Overlord guardrail prevention benchmark",
            "",
            ...
            "## Summary",
            "",
            "| Metric | Baseline (unchecked delete + rebuild) | With Overlord | Savings |",
            "|--------|---------------------------------------|---------------|---------|",
            ...
            f"**Overlord blocked preflight:** {c.get('overlord_blocked_action')}",
