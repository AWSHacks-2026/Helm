...
def build_cost_comparison(
    ...
    overlord_usage: dict[str, Any],
) -> dict[str, Any]:
    """Cost-focused comparison; Sonnet-heavy Overlord paths vs Haiku baseline."""
    b_cost = cost_from_usage(baseline_usage)
    o_cost = cost_from_usage(overlord_usage)
    delta = b_cost - o_cost
    savings_pct = int(100 * delta / b_cost) if b_cost > 0 else 0
    return {
        "baseline_cost_usd": round(b_cost, 6),
        "overlord_cost_usd": round(o_cost, 6),
        "cost_delta_usd": round(delta, 6),
        "cost_savings_pct": savings_pct,
        "overlord_beats_cost": o_cost < b_cost,
        "baseline_cost_display": format_usd(b_cost),
        "overlord_cost_display": format_usd(o_cost),
        "cost_note": (
            "Overlord ledger shows $0 — merge may have used AgentCore Runtime (untracked)."
            if o_cost == 0 and overlord_usage.get("call_count", 0) == 0
            else None
        ),
    }
