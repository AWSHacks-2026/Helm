...
def test_intent_benchmark_detects_conflict_and_unifies():
    ...
    assert c["overlord_stops_conflict"] is True
    ...
    assert c["overlord_agents_proceeding"] == 1
    assert c["overlord_beats_cost"] is True
    assert c["cost_savings_pct"] > 0


def test_intent_resolution_contract_fields():
    raw = run_intent_benchmark()["overlord"]["resolution"]
