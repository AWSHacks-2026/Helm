#!/usr/bin/env python3
"""Verify AWS credentials and optional Overlord cloud resources."""
...
def check_bedrock_invoke() -> dict:
    if os.getenv("OVERLORD_MOCK_BEDROCK") == "1":
        return {
            "name": "bedrock_invoke",
            "passed": True,
            "detail": "skipped (OVERLORD_MOCK_BEDROCK=1)",
        }
...
def check_memory() -> dict:
    ...
    use_local = os.getenv("OVERLORD_USE_LOCAL_MEMORY", "true").lower() == "true"
    if use_local or not memory_id:
        return {
            "name": "agentcore_memory",
            "passed": True,
            "detail": "local mode (set AGENTCORE_MEMORY_ID + OVERLORD_USE_LOCAL_MEMORY=false to test cloud)",
        }
    try:
...
def check_policy_env() -> dict:
    ...
    use_local = os.getenv("OVERLORD_USE_LOCAL_POLICY", "true").lower() == "true"
    if use_local:
        detail = "local bridge (Python Cedar semantics)"
    elif engine_id:
        detail = f"engine {engine_id} configured (Gateway ENFORCE optional)"
    else:
        detail = "OVERLORD_USE_LOCAL_POLICY=false but no AGENTCORE_POLICY_ENGINE_ID"
...
def main() -> int:
    ...
    if args.bedrock or os.getenv("OVERLORD_MOCK_BEDROCK", "0") != "1":
        checks.append(check_bedrock_invoke())
        checks.append(check_bedrock_agent_invoke())
    if args.memory:
        os.environ["OVERLORD_USE_LOCAL_MEMORY"] = "false"
