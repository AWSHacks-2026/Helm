#!/usr/bin/env python3
"""Create AgentCore Memory and/or Policy Engine resources for Overlord."""
...
CEDAR_PATH = ROOT / "backend" / "bedrock" / "policies" / "overlord_coordination.cedar"
...
def _sanitize_name(prefix: str, suffix: str) -> str:
    ...
    if not base or not base[0].isalpha():
        base = f"overlord_{base}" if base else "overlord_resource"
    return base[:48]
...
def create_memory(name: str) -> str:
    ...
    if hasattr(client, "create_memory_and_wait"):
        resp = client.create_memory_and_wait(
            name=name,
            description="Overlord multi-agent session history (intents, actions, decisions)",
        )
    else:
...
def create_policy_engine(name: str) -> str:
    ...
    resp = client.create_policy_engine(
        name=name,
        description="Overlord coordination: file overlap, intent clash, reverse peer work",
        clientToken=token,
    )
...
def upload_cedar_policies(engine_id: str) -> list[str]:
    ...
    for idx, block in enumerate(blocks):
        ...
        policy_name = f"overlord_coordination_{idx}"
        client.create_policy(
            name=policy_name,
            policyEngineId=engine_id,
            validationMode="IGNORE_ALL_FINDINGS",
            description=f"Overlord coordination rule {idx}",
...
def print_env_snippet(memory_id: str | None, engine_id: str | None) -> None:
    lines = ["# Paste into overlord/.env", f"AWS_REGION={REGION}"]
    if memory_id:
        lines.extend(
            [
    ...
    lines.append("OVERLORD_MOCK_BEDROCK=0")
    print("\n".join(lines))
...
def main() -> int:
    ...
    parser.add_argument("--name-prefix", default="overlord_hackathon")
    parser.add_argument(
        "--skip-policies",
        action="store_true",
