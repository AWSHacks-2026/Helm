#!/usr/bin/env python3
"""Generate browser-runnable static commerce sites with and without Overlord."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate and score static commerce sites with and without Overlord."
    )
    parser.add_argument(
        "--agent-counts",
        type=int,
    ...
    parser.add_argument(
        "--allow-mock",
        action="store_true",
        help="Allow OVERLORD_MOCK_BEDROCK=1 for local smoke tests.",
    )
    parser.add_argument(
        "--no-progress",
        action="store_true",
    ...
    print(
        "View a site with: "
        f"python -m http.server 5174 --directory {config.output_dir / 'with-overlord' / f'N{view_count}'}"
    )
    return 0
