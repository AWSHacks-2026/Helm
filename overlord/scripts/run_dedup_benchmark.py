#!/usr/bin/env python3
"""Compare baseline duplicate agent work vs Overlord deduplication."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Overlord deduplication benchmark")
    parser.add_argument(
        "--scenario",
    ...
    print(
        ...
        f"({c['overlord_full_implementation_runs']} full impls) | "
        f"saved {c['cost_savings_pct']}% cost "
        f"({c['baseline_tokens']}→{c['overlord_tokens']} tok), "
