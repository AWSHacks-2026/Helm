#!/usr/bin/env python3
"""Compare six-agent Haiku merge thrashing vs Helm fleet arbitration."""
...
def main() -> int:
    ...
    print(
        ...
        f"({c['helm_arbitration_calls']} arbitration) | "
        f"saved {c['cost_savings_pct']}% cost, {c['time_savings_pct']}% time, "
        f"score {c['baseline_mean_score']}% → {c['helm_mean_score']}%"
    )
