...
def test_run_static_site_benchmark_mock_writes_all_agent_counts(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "with-helm" / "N8" / "index.html").exists()
    assert (tmp_path / "without-helm" / "N8" / "index.html").exists()
    n8_without = next(
        result
        for result in results
        if result.agent_count == 8 and result.mode == "without-helm"
    )
    n8_with = next(
        result
        for result in results
        if result.agent_count == 8 and result.mode == "with-helm"
    )
    assert n8_with.duplicate_modules < n8_without.duplicate_modules
    assert n8_with.quality_score > n8_without.quality_score
    with_html = (tmp_path / "with-helm" / "N8" / "index.html").read_text(
        encoding="utf-8"
    )
...
def test_rich_mock_generation_records_file_conflict_metrics(tmp_path, monkeypatch):
    ...
    without = next(result for result in results if result.mode == "without-helm")
    with_helm = next(result for result in results if result.mode == "with-helm")
    manifest = json.loads(
        (tmp_path / "without-helm" / "N4" / "manifest.json").read_text(
            encoding="utf-8"
        )
    )

    assert without.conflict_markers > 0
    assert with_helm.conflict_markers == 0
    assert without.quality_score < with_helm.quality_score
    assert manifest["merge_conflicts"] >= 1
    assert (tmp_path / "without-helm" / "N4" / "agent-work").exists()
...
def test_static_site_generation_script_exists():
    ...
    script = helm_root / "scripts/benchmark_static_site_generation.py"
