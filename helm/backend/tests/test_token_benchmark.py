...
def test_benchmark_constants_match_demo_spec():
    ...
    assert DEFAULT_OUTPUT_DIR == HELM_ROOT / "demo"
    assert TEST_PROMPT == (
...
def test_benchmark_config_defaults_are_frozen():
    ...
    assert config.output_dir == HELM_ROOT / "demo"
...
def test_benchmark_row_to_dict_and_frozen_behavior():
    row = BenchmarkRow(
        agent_count=2,
        without_helm_tokens=100,
        with_helm_tokens=60,
        without_helm_seconds=1.25,
        with_helm_seconds=0.5,
        without_helm_build_rate=0.0,
        with_helm_build_rate=1.0,
        without_helm_conflicting_edits=1,
        with_helm_conflicting_edits=1,
        without_helm_reverted_commits=0,
        with_helm_reverted_commits=1,
    )
...
def test_benchmark_script_exists():
    script = HELM_ROOT / "scripts/benchmark_helm_tokens.py"
...
def test_build_helm_agents_combines_intents_for_arbitrate():
    agent_a, agent_b = build_helm_agents(4)
...
def test_realistic_overlap_helm_pairs_are_cluster_leads():
...
def test_count_reverted_commits_counts_discarded_outputs():
    ...
    assert count_reverted_commits(agent_count=4, with_helm=True) == 0
    assert count_reverted_commits(agent_count=4, with_helm=False) == 0
...
def test_run_without_helm_uses_pairwise_haiku_calls(monkeypatch):
    ...
    result = token_benchmark.run_without_helm(
        agent_count=4,
        max_tokens=300,
...
def test_run_without_helm_uses_progress_when_enabled(monkeypatch):
    from agents import token_benchmark
    ...
    token_benchmark.run_without_helm(
        agent_count=2,
        max_tokens=300,
        profile=token_benchmark.WORST_CASE_PROFILE,
        show_progress=True,
    ...
    assert progress_calls == [
        {
            "total": 2,
            "desc": "Without Helm worst-case N=2",
            "enabled": True,
        }
    ]


def test_run_without_helm_rejects_missing_usage_tokens(monkeypatch):
    from agents import token_benchmark
    ...
    with pytest.raises(RuntimeError, match="positive Bedrock usage token counts"):
        token_benchmark.run_without_helm(agent_count=2, max_tokens=300)


def test_run_with_helm_uses_existing_arbitrate(monkeypatch):
    from agents import token_benchmark
    ...
    def fake_arbitrate(agent_a, agent_b, **kwargs):
        ...
        return {
            ...
            "reasoning": "Helm assigns one lead and coordinates the rest.",
    ...
    result = token_benchmark.run_with_helm(
        agent_count=4,
        profile=token_benchmark.WORST_CASE_PROFILE,
    )
...
def test_run_with_helm_uses_progress_when_enabled(monkeypatch):
    from agents import token_benchmark
    ...
    assert progress_calls == [
        {"total": 3, "desc": "With Helm worst-case N=4", "enabled": True}
    ]


def test_run_with_helm_rejects_missing_usage(monkeypatch):
    from agents import token_benchmark
    ...
    with pytest.raises(RuntimeError, match="tracked Bedrock arbitration"):
        token_benchmark.run_with_helm(agent_count=2)


def test_run_with_helm_rejects_malformed_usage(monkeypatch):
    from agents import token_benchmark
...
def test_runners_reject_single_agent():
    ...
    with pytest.raises(ValueError, match="agent_count must be at least 2"):
        token_benchmark.run_without_helm(agent_count=1, max_tokens=300)
    with pytest.raises(ValueError, match="agent_count must be at least 2"):
        token_benchmark.run_with_helm(agent_count=1)
...
def test_run_benchmark_matrix_allows_mock_when_explicit(monkeypatch):
    ...
    assert rows == [
        BenchmarkRow(
            agent_count=2,
            without_helm_tokens=30,
            with_helm_tokens=15,
            without_helm_seconds=0.25,
            with_helm_seconds=0.1,
            without_helm_build_rate=0.0,
            with_helm_build_rate=1.0,
            without_helm_conflicting_edits=1,
            with_helm_conflicting_edits=1,
...
def test_run_benchmark_matrix_uses_realistic_profile_by_default(monkeypatch):
    ...
    monkeypatch.setattr(token_benchmark, "run_without_helm", fake_without)
    monkeypatch.setattr(token_benchmark, "run_with_helm", fake_with)
...
def test_write_results_json(tmp_path):
    ...
    row = BenchmarkRow(
        ...
        without_helm_tokens=100,
        with_helm_tokens=50,
        without_helm_seconds=2.0,
        with_helm_seconds=1.0,
        without_helm_build_rate=0.0,
        with_helm_build_rate=1.0,
        without_helm_conflicting_edits=1,
        with_helm_conflicting_edits=1,
    ...
    assert path.name == "helm-token-benchmark.json"
    assert path.exists()
...
def test_write_results_json_includes_profile_metadata(tmp_path):
    ...
    assert path.name == "helm-token-benchmark-realistic-overlap.json"
...
def test_save_benchmark_figure(tmp_path):
    ...
    rows = [
        ...
        BenchmarkRow(
            agent_count=4,
            without_helm_tokens=300,
            with_helm_tokens=120,
            without_helm_seconds=5.0,
            with_helm_seconds=2.0,
            without_helm_build_rate=0.0,
            with_helm_build_rate=1.0,
            without_helm_conflicting_edits=1,
            with_helm_conflicting_edits=1,
            without_helm_reverted_commits=0,
            with_helm_reverted_commits=3,
        ),
    ]

    path = save_benchmark_figure(rows, tmp_path)

    assert path.name == "helm-token-benchmark.png"
    assert path.exists()
...
def test_save_benchmark_figure_uses_profile_filename(tmp_path):
    ...
    assert path.name == "helm-token-benchmark-worst-case.png"
    assert path.exists()
    assert path.stat().st_size > 0
