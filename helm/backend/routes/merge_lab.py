...
def _resolve_helm(scenario_name: str, scenario: dict[str, Any]) -> dict[str, Any]:
...
def _run_strategy(
    ...
    if strategy == "helm":
        ...
        raw = _resolve_helm(scenario_name, scenario)
...
def compare_merge_strategies(scenario_name: str) -> dict[str, Any]:
    """Run Helm vs naive baselines; return pass/score for each strategy."""
    if scenario_name not in get_merge_scenario_names():
        raise HTTPException(status_code=404, detail="Merge scenario not found")
    ...
    strategies = ["helm", "pick_agent_a", "pick_agent_b", "dual_edit_markers"]
    results = [_run_strategy(s, scenario_name, scenario, acceptance) for s in strategies]

    helm = next(r for r in results if r["strategy"] == "helm")
    best_naive = max(
        (r for r in results if r["strategy"] != "helm"),
    ...
    return {
        ...
        "summary": {
            ...
            "helm_beats_naive": helm["evaluation"]["score"] > best_naive["evaluation"]["score"],
            "score_delta": helm["evaluation"]["score"] - best_naive["evaluation"]["score"],
        },
        "mcp_hint": {
            "tool": "helm_resolve_conflict",
