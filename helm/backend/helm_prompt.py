...
def build_multi_agent_guardrail_prompt(
    ...
    return f"""You are Helm. A proactive guardrail BLOCKED an agent action in a multi-agent fleet.
...
def build_task_deduplication_prompt(agent_a: dict, agent_b: dict) -> str:
    return f"""
You are Helm, a supervisor for multiple AI coding agents.
...
def build_multi_agent_deduplication_prompt(agents: dict[str, dict]) -> str:
    ...
    agent_list = ", ".join(f'"{aid}"' for aid in agents)
    return f"""
You are Helm, a supervisor for multiple AI coding agents on one codebase.
...
def build_multi_agent_merge_prompt(
    ...
    return f"""
You are Helm, resolving MERGE CONFLICTS across multiple AI coding agents.
...
def build_single_file_merge_prompt(file_path: str, agents: dict[str, dict]) -> str:
    ...
    return f"""
You are Helm, resolving a MERGE CONFLICT on `{file_path}`.
