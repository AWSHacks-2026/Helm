"""Six-agent merge conflict benchmark: Haiku chain vs Helm fleet arbitration."""
...
from helm import _arbitrate_file_group, arbitrate_fleet
...
def run_baseline_path(
    ...
    """Agents sequentially merge-fix on each shared file (no Helm)."""
            ...
            if usage_meta:
                ledger.add(
                    InvokeUsage(
                        ...
                        role=f"helm-merge-{file_path.replace('/', '-')}",
...
def run_merge_fleet_benchmark(
    ...
    helm = run_helm_path(
        scenario, agents, file_paths, grouped, acceptance_by_file, helm_ledger
    )
    ...
    merge_calls_saved = baseline["merge_fix_calls"] - helm.get("arbitration_calls", 1)
    ...
    return {
        ...
        "comparison": {
            ...
            **build_cost_comparison(baseline["usage"], helm["usage"]),
            ...
            "helm_arbitration_calls": helm["arbitration_calls"],
            "merge_fix_calls_avoided": max(0, merge_calls_saved),
            "baseline_mean_score": baseline["mean_score"],
            "helm_mean_score": helm["mean_score"],
            "helm_beats_quality": helm["mean_score"] >= baseline["mean_score"],
            "baseline_passed_all": baseline["passed_all"],
            "helm_passed_all": helm["passed_all"],
        },
    }
