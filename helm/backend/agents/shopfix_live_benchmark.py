...
from agents.shopfix_execution_plan import (  # noqa: F401 — re-export for tests / demo matrix
...
def _paths_needing_resolution(
    sandbox: GitSandbox,
    by_file: dict[str, list[AgentAssignment]],
) -> list[str]:
...
def _finish_sandbox(
    ...
    if paths:
        _resolve_all_conflicts(sandbox, paths, by_file, ledger)
...
def run_shopfix_live_pair(
    ...
    comparison["token_savings_pct"] = int(100 * (b_tok - h_tok) / b_tok) if b_tok > 0 else 0
...
def format_summary(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"  tokens:   baseline={c.get('baseline_total_tokens', 0):,} helm={c.get('helm_total_tokens', 0):,} "
        f"({c.get('token_savings_pct', 0)}% Δ)",
