...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    max_cost = max(c["baseline_cost_usd"], c["helm_cost_usd"], 1e-9)
    max_ms = max(c["baseline_resolution_time_ms"], c["helm_resolution_time_ms"], 1)
    ...
    lines.extend(
        [
            ...
            "## Summary",
            "",
            "| Metric | Baseline (Haiku merge chain) | With Helm | Savings |",
            "|--------|------------------------------|---------------|---------|",
            ...
            f"| Wall time (ms) | {c['baseline_resolution_time_ms']:,} | {c['helm_resolution_time_ms']:,} | {c['time_savings_pct']}% |",
            f"| Merge-fix / arbitration calls | {c['baseline_merge_fix_calls']} | {c['helm_arbitration_calls']} | {c['merge_fix_calls_avoided']} avoided |",
            f"| Mean quality score | {c['baseline_mean_score']}% | {c['helm_mean_score']}% | — |",
            f"| All files pass acceptance | {c['baseline_passed_all']} | {c['helm_passed_all']} | — |",
            "",
            "## Comparison chart (cost & time)",
            "",
            "```",
            _bar_cost("Baseline $", c["baseline_cost_usd"], max_cost),
            _bar_cost("Helm $", c["helm_cost_usd"], max_cost),
            _bar("Baseline ms", c["baseline_resolution_time_ms"], max_ms),
            _bar("Helm ms", c["helm_resolution_time_ms"], max_ms),
            "```",
            "",
